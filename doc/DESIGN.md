...
### 11.1 主界面（Dashboard）
```
Row 1  [ 欢迎卡 col-12：左 IconAvatar + 右文案（日期 / 问候 / 当前用户） ]
Row 2  [ StatTile×4（每个 col-3）——今日收入 / 支出 / 净额 / 待出餐 ]
Row 3  [ 余餐提醒横条 col-12（仅在有待续卡会员时显示） ]
Row 4  [ 快捷操作 Bento：左列（会员档案 + 散客目录，2 行 1 列）| 右列（每日订餐，2 行高） ]
Row 5  [ 次级入口双列（订单统计 / 财务记账 / 员工名单 / 权限管理） ]
Row 6  [ 当前用户 col-12（避免尾部半列留白） ]
```
手机折叠后：`col-3→col-6`（一行两个），`col-6→col-12`（堆叠）。

主界面与登录页保持同语言：
- 左图标右文案结构（建立识别一致性）
- 重点入口整宽，次级入口双列，避免所有卡片同权重导致视觉疲劳
...
Row 0  [ 二级 Tab：总览 / 录入 / 批量录入 / 赠送餐 / 零售 /（出餐分组：出餐·送餐·快递） ]
Row 1  [ 页顶 meta：今日日期 |「每次加载」档位（总览等列表 Tab 显示；录入/零售等全屏可隐藏该行）]
Row 2  [ 总览：日期卡（今天/明天快捷 + DatePicker）]
Row 3  [ 总览汇总白卡 · 双行 ]
        · 上行：总餐数 | 午餐 | 晚餐 | 散餐（不含已取消；散餐为 card 空的子集）
        · 下行：待出餐 | 已出餐 | 已送达（按 status 动态变化）
Row 4  [ SectionList：午 / 晚分组；每行 OrderRow：姓名+状态 chip；灰字元信息串，其中「N 份」本单份数蓝色加粗；个人忌口/订单备注橙色保留 ]
Row 5  [ 录入：凭证 + 会员/散客表单… ]
```

**批量录入（会员 · 批量模式）**  
- **顺序**：「已加入」名单与午/晚并排步进器在上；**「搜索」** 标题与搜索框、结果列表在下。  
- **已加入卡片**：左栏姓名 + **同一行**接「院内·卡种·剩份」灰字（窄屏可换行）；手机/微信各一行；**删除**固定最右、纵向与左块居中；平板宽 **≥768** 时名单 **三列**网格。  
- **午/晚**：`LunchDinnerQtyPair` 单行两列步进器；批量内可用 `dense` 减内边距。  
- **搜索**：结果行含手机、微信；与单人录入搜索结果一致。

**总览列表行（OrderRow）**  
- 元信息为「 · 」串，**本单点餐份数**（`quantity` +「份」）单独 **高亮色 + 加粗**，其余为次要灰字。

**「零售」Tab（其他产品销售）** 与订餐会员**无绑定**：整页纵向 ScrollView，白底卡片分区（与订餐页 `orderScreenStyles` 节奏一致，不必套 Bento）：

```
Header [AppHeader(每日订餐) 标题不变]
Tab    [ … 选中「零售」 ]
Block A [ 记一笔零售 ]
        · DatePicker（业务日 ≤ 今天）
        · 商品：横向 Chip 单选（仅 `is_active` 商品；无商品时橙色提示去目录新增）
        · 数量（整数）、总价（元）、收款人（横向滚动 Chip，数据源同开卡 Modal 的 `/api/users`）
        · 备注可选；主按钮「确认入账」品牌色
Block B [ 本日已录 ] — 列表展示当日 `misc_retail_income` 摘要（金额绿色 + description 多行截断）；右上「刷新」
Block C [ 商品目录 ] — 行：名称 + 描述 + 铅笔（编辑）+ 眼睛/闭眼（停用/启用）；右上「+ 新增」打开 Modal（名称 + 多行描述）
```

Modal：全屏半透明遮罩 + 居中白卡；底部「取消 / 保存」；保存态禁用背景点击关闭。
...
Row 2  [ 筛选条 col-12 ]
Row 3  [ 明细列表 col-12 ]  ← 每条 GlassSurface：日期 + 类型/分类 Chip + 金额；若有 collector_user_id，在 description 下增加一行 footnote 字重「收款：{姓名}」
FAB    [ +新增支出（右下浮动） ]
```

**其他产品收入 Modal**：保留作**无价目表**时的临时一笔（自由文本摘要 + 金额 + 日期）；副文案注明有价目表时优先走「每日订餐 → 零售」。
...
### 11.7 登录
```
Center  [ Hero Row：左 Logo（IconAvatar 大号） | 右 Badge + 标题 + 副标题 ]
        [ GlassSurface col-12 maxWidth 400 ]
          [ 用户名（左 icon） ]
          [ 密码（左 icon + 右显隐） ]
          [ PrimaryButton 登录 ]
```
...
- 输入区必须有图示前缀（用户 / 锁）
- 密码框必须支持显隐切换
- 错误提示优先使用 glass `danger tint`，避免纯红底告警
- 登录后首页默认不显示“工作台”文案，仅保留日期 + 问候语
...
## 16. v3 落地补充（2026-04-25）

本轮上线后的界面行为，以代码实现为准，补充如下：

- 主界面入口保持双主路径：`录入/总览` 与 `出餐/配送` 分离，降低一线操作员点击路径。
- 首页恢复 `财务流水` 与 `订餐数据` 两类入口，维持运营视角与执行视角并行。
- 订餐数据页简化为总览视图：保留周/月/年切换与状态快照分布，移除细粒度筛选与冗余榜单。
- 财务页结构分布拆分为“收入结构 / 支出结构”两块，并将明细窗口固定为最近 7 天，减少噪音。
- 会员详情卡操作明确为双按钮：`续卡` + `升级卡片` 并存，低余量仅作为提示，不隐藏任一操作。
- 余餐提醒页将“需跟进人数”从 Header 副标题下沉为总览卡片，统一与首页信息节奏。
- 对首页关键图标执行光学校正（0.5~1px 级偏移）与固定槽位对齐，优化 iPhone 视觉基线一致性。

---

## 17. v3 落地补充（2026-04-28）

本轮针对一线配送与小屏一致性补充如下：

- 每日订餐「总览」增加日期切换能力：默认今天，支持快捷切到明天，并支持任意日期选择。
- DatePicker 行内标签统一单行策略（`numberOfLines=1` + 固定最小宽度），避免 iPhone 小屏换行导致基线错位。
- 顶部返回键安全区统一：页面一律使用 `react-native-safe-area-context` 的 `SafeAreaView edges=['top']`，消除“同样是返回键但位置不齐”的视觉问题。
- 送餐场景新增“送餐失败并退餐”：仅允许对 `已出餐` 订单执行，必须填写失败原因（含快速选项），提交后自动执行原子退餐并记录审计。
- 失败单在 UI 中保留“送餐失败”识别标签；取消原因中保留 `配送失败：...` 文字，方便客服回溯与复盘。

---

## 18. v3 交互细化（2026-04-28 晚）

- 状态面板中，`已出餐` 场景下将“已取消”动作直接替换为“送餐失败并退餐”，避免操作语义混淆。
- 送餐失败相关按钮统一危险红色语义（按钮底色/文字/选中态），与“终态回滚”风险等级一致。
- 状态按钮视觉改为“图标 + 文本同一行”，提升小屏阅读效率并降低误读。
- 出餐卡片信息区重排为“餐别 pill + 份数 pill + 卡种/散餐”，减少大字号数字带来的视觉跳动。
- 会员档案筛选区顺序调整为“筛选 Tab → 搜索框 → 加载档位”，符合先定范围再检索的阅读习惯。
- 每日订餐页将“每次加载”并入日期行右侧（`justify-content: space-between`），减少纵向占高。

---

## 19. v3 落地补充（2026-05-08）— 其他零售（非会员绑定）

- **入口**：每日订餐 → 管理分组 → **「零售」** Tab（`Ionicons` `pricetag-outline`），与总览/录入/批量/赠送餐同级；出餐/配送分组不变。
- **数据**：销售写入 `finance_entries`，`category=misc_retail_income`，可带 `retail_product_id`、`quantity`、`collector_user_id`；**不**写 `ref_card_id` / `ref_order_id`。
- **统计**：后端将 `misc_retail_income` 计入 `GET /api/finance` 返回的 `summary.realized_income`（与送餐履约收入同属「已入账现金」口径，便于日结净额对账）。
- **API 路径**：移动端商品目录默认请求 **`/api/finance/retail-products`**（与财务同域）；亦保留顶层 **`/api/retail-products`** 兼容。
- **销售入账**：`POST /api/finance/retail-product-sale`，JSON 字段 `entry_date`、`product_id`、`quantity`、`amount`（元）、`collector_user_id`、`note?`（与 `@repo/shared` 的 `retailProductSaleCreateSchema` 一致）。

---

## 20. 设计文档 PDF 导出（含 Mermaid）

- **命令**（仓库根目录）：`pnpm docs:pdf`（实现于 `@meal/scripts`，依赖 Puppeteer + Mermaid CDN）。
- **合并范围**：`DESIGN.md`、`PROCESS.md`、`ARCHITECTURE_DIAGRAMS.md`、`LINEAR.md`、`V5_XLSM_IMPORT_RUNBOOK.md` 按顺序拼成一份 HTML，再将 fenced **` ```mermaid `** 代码块替换为 `<div class="mermaid">`，由浏览器内 **Mermaid.js** 渲染为 SVG 后打印 PDF。
- **输出路径**：`doc/pdf/meal-design-documentation.pdf`（默认 **gitignore**，避免二进制频繁入库；需要归档时本地生成后另存）。
- **注意**：首次运行会下载 Chromium；无网络或 CI 无浏览器时需自备环境或跳过该步骤。
