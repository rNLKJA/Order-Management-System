 ...
 * 根据"会员是否院内 + 卡代码"取卡规格。
 */
export function getCardSpec(isHospital, code) {
    if (isHospital) {
        const hospitalCatalog = CARD_CATALOG.hospital;
        return hospitalCatalog[code] ?? null;
    }
    const regularCatalog = CARD_CATALOG.regular;
    return regularCatalog[code] ?? null;
}
/**
 * 列出某价目表下所有卡（按总价升序）。
 */
export function listCards(isHospital) {
 ...
 */
export function listUpgradeOptions(isHospital, currentPaidAmount) {
