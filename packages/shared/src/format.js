...
export function formatDate(date, tz = DEFAULT_TZ) {
...
export function formatDateTime(date, tz = DEFAULT_TZ) {
    const d = typeof date === 'string' ? new Date(date) : date;
    const fmt = new Intl.DateTimeFormat('zh-CN', {
        timeZone: tz,
        year: 'numeric',
...
/** "10 餐" / "剩余 4 餐" */
export function formatMeals(qty, prefix = '') {
 ...
 * 例：牙巴(13985739933)
 */
export function buildUid(nickname, name, phone) {
 ...
 */
export function businessDate(date, tz = DEFAULT_TZ) {
    const d = typeof date === 'string' ? new Date(date) : date;
