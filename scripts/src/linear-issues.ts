...
## 验收标准
- [x] POST /api/members 创建 + uid 自动拼
- [x] GET /api/members 搜索
- [x] PATCH /api/members/:id 编辑
- [x] DELETE /api/members/:id 仅 admin + 无引用才允许
- [x] Expo 会员列表页 + 创建页 + 详情页骨架
- [x] 重复手机号 Dialog 提示（不硬阻）
...
## 验收标准
- [x] POST /api/cards 新购 + 自动 income
- [x] POST /api/cards/:id/upgrade 补差价 + 继承已用
- [x] 升级校验 newCat.totalPrice > old.paid_amount 否则 422
- [x] 耗尽 remaining=0 自动置 exhausted
- [x] 换卡不受旧卡等级限制
- [x] Expo Modal 价格网格 + 收款人/录入者下拉 + 计算应收/补差价/新剩余
...
## 验收标准
- [x] POST /api/orders 接 { member_id, order_date, lunch_qty, dinner_qty, notes }，拆 1-2 条
- [x] 合并校验 total_qty <= card.remaining，超了 422 INSUFFICIENT_MEAL_BALANCE
- [x] 扣到 0 → exhausted + 响应带"提醒续卡"标志
- [x] 散餐 amount=35×qty + 自动 FinanceEntry(income, ad_hoc)
- [x] PATCH /api/orders/:id 原子回滚重扣
- [x] Idempotency-Key header 防重复提交
- [x] Expo 录入 Modal + 会员详情的订餐记录表
...
## 验收标准
- [x] FinanceEntry 建新时自动分类（hospital_sub / regular_sub / ad_hoc / manual_expense）
- [x] POST /api/finance/expense 手动录支出
- [x] GET /api/finance 列表支持日期/type/category 筛选
- [x] PATCH /api/finance/:id 编辑金额/备注/分类
- [x] DELETE 仅 admin
- [x] Expo 财务页：分类汇总卡 + 明细表 + 新增支出 Modal
...
## 验收标准
- [x] 订阅记录表倒序显示所有历史卡，状态 Badge
- [x] 订餐记录表显示餐别 Badge + 状态 Badge + 录入者 + 打餐人 + 送达人（commit d75a98c）
- [x] 累计数据聚合正确
- [x] 操作按钮按状态切换（无 active 卡 → "购买新卡"；有 → "升级 / 录入用餐"）
...
`,
  ...
  // ==================== Phase 4+ ====================
  {
    ...
    body: `## 目标
手机专用打餐视图：4 Tab（录入 / 总览 / 出餐 / 送餐）+ 餐别 / 配送区筛选 + 完成出餐 / 确认送达 / 取消；delivered 终态锁定；Haptics 振动。

## 落地说明
未单独建 packing 路由，实现方式是把 4 Tab 合并进 apps/mobile/app/(app)/orders/index.tsx（entry/overview/prep/delivery），复用同一套数据和 Mutation；桌面端隐藏 prep/delivery Tab。

## 验收标准
- [x] 4 Tab（entry / overview / prep / delivery）
- [x] 餐别 / 院内外 Chip 筛选
- [x] pending → fulfilled → delivered 流转
- [x] cancel 原子冲销（fulfilled 允许，delivered 终态锁）
- [x] 成功反馈 Haptics
- [ ] 离线缓存（Phase 4+ 可选，暂未实现）

## 关联
- plan §9、§25 step 8
- apps/api/src/routes/orders.ts PATCH /:id/status + /:id/cancel
`,
  },
  {
...
## 验收标准
- [x] cards PATCH + audit（commit 2fc5821）
- [x] finance PATCH + audit（commit f080a14 / 2fc5821）
- [x] orders PATCH notes / status（apps/api/src/routes/orders.ts）
- [x] delivered / cancelled 终态锁定（services/orders.ts OrderLockedDeliveredError）
- [x] GET /api/audit 列表（commit 2fc5821）

## 关联
- plan §17、§25 step 9
- commits aab8343 / 2fc5821
`,
  },
  {
...
## 验收标准
- [x] doc/DESIGN.md v3（2026-04-23 重写），主题改为 Glassmorphism + Bento + Apple/Material
- [x] 共享原语：GlassSurface / BentoGrid / AppHeader / StatusChip / StatTile（apps/mobile/components/ui/*）
- [x] Tokens：apps/mobile/theme/paperTheme.ts（COLORS / SPACING / TYPE）
- [ ] 5 张未落地页面样板（tomorrow/report/summary/export）—— 对应功能尚未上线，待 MEA-18~22 完成后补
